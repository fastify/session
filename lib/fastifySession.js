...
const getExpires = require('./cookie').getExpires
let Signer

function session (fastify, options, next) {
  if (!Signer) {
    Signer = require('@fastify/cookie').Signer
  }
  ...
  function decryptSession (sessionId, options, request, done) {
    ...
    options.store.get(decryptedSessionId, (err, session) => {
      ...
      if (session && session.cookie && 'expires' in session.cookie) {
        session.cookie.expires = getExpires(cookieOpts)
      }
  ...
  function ensureDefaults (options) {
    ...
    options.signer = typeof options.secret === 'string' || Array.isArray(options.secret) ? new Signer(options.secret, options.algorithm) : options.secret
