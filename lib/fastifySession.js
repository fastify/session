...
function preValidation (options) {
  ...
  return function handleSession (request, reply, done) {
    ...
    if (!sessionId) {
      newSession(secret, request, cookieOpts, options.idGenerator, done)
    } else {
        ...
        options.store.get(decryptedSessionId, (err, session) => {
          ...
          if (session && session.expires && session.expires <= Date.now()) {
            options.store.destroy(
              sessionId,
              getDestroyCallback(secret, request, reply, cookieOpts, options.idGenerator, done)
            )
            return
          }
...
function getDestroyCallback (secret, request, reply, cookieOpts, idGenerator, done) {
...
function newSession (secret, request, cookieOpts, idGenerator, done) {
  request.session = new Session(cookieOpts, secret, {}, idGenerator)
  done()
}
